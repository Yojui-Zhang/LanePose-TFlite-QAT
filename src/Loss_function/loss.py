...
def distill_loss_pose(y_teacher, y_student,
    ...
    box_s_logits, obj_s, cls_s, kxy_s_logits, ks_s = _split_outputs(y_student, num_cls, num_kpt, kpt_vals)

    # NEW: 將 student 的 box 和 kxy logit 轉換為機率
    box_s = tf.nn.sigmoid(box_s_logits)
    kxy_s = tf.nn.sigmoid(kxy_s_logits)

    # ... w_obj 的計算保持不變 ...
    p_t = tf.nn.softmax(cls_t)
    w_obj = tf.reduce_max(p_t, axis=-1, keepdims=True)

    # Box: L1，加權 (現在是在兩個機率之間計算)
    ...
    p_t_cls = tf.nn.softmax(cls_t) # 避免重複命名
    p_s_cls = tf.nn.softmax(cls_s)
    cls_l = kl_div_weighted(p_t_cls, p_s_cls, weight=w_obj)

    # KPT (x,y): L1，加權 (現在是在兩個機率之間計算)
    kxy_l = tf.reduce_mean(w_obj[..., None] * tf.abs(kxy_t - kxy_s))
    # KPT (score): BCE（logits），加權
