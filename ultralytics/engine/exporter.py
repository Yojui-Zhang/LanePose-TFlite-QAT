...
class Exporter:
    ...
    def __call__(self, model=None) -> str:
        ...
        metadata_date = os.getenv("ULTRALYTICS_EXPORT_DATE") or datetime.now().isoformat()
        self.metadata = {
    ...
    def _add_tflite_metadata(self, file):
        ...
        metadata_json = json.dumps(self.metadata, indent=2, sort_keys=True, ensure_ascii=False)
        zip_timestamp = (1980, 1, 1, 0, 0, 0)  # ZIP minimum valid timestamp for deterministic archives.
        metadata_date = self.metadata.get("date")
        if isinstance(metadata_date, str):
            try:
                dt = datetime.fromisoformat(metadata_date.replace("Z", "+00:00")).replace(tzinfo=None)
                year = min(max(dt.year, 1980), 2107)  # ZIP date range.
                zip_timestamp = (year, dt.month, dt.day, dt.hour, dt.minute, dt.second)
            except ValueError:
                pass

        metadata_entry = zipfile.ZipInfo(filename="metadata.json", date_time=zip_timestamp)
        metadata_entry.compress_type = zipfile.ZIP_DEFLATED
        metadata_entry.create_system = 0
        metadata_entry.external_attr = 0

        with zipfile.ZipFile(file, "a", zipfile.ZIP_DEFLATED) as zf:
            zf.writestr(metadata_entry, metadata_json)
