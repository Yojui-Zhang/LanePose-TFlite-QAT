...
# ===========================================================================================================================
#                     v = kpts[j].conf          # Tensor，形狀通常是 [..., K]
#                     v.clamp_min_(1)
#                     kpt = torch.cat((kpts[j].xyn, v[..., None]), dim=-1)
# ===========================================================================================================================

                    # v: 原本的信心值 (… , K)
                    v = kpts[j].conf                       # 形狀通常是 (..., 15)

                    # 建立全為 1 的可見度 / 置信度向量
                    vis = torch.ones_like(v, dtype=v.dtype)
#                     vis = (v > 0.1).to(v.dtype)

                    # 把第 9、14、15 個點設為 0（注意 0-based 索引）
                    vis[..., 8]  = 0
                    vis[..., 13] = 0
                    vis[..., 14] = 0

                    # 拼接成 (… , K, 3) = (x, y, v)
                    kpt = torch.cat((kpts[j].xyn, vis[..., None]), dim=-1)
